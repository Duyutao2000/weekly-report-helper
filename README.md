...
- **任务管理** — 按项目分组的树形任务看板，支持新增/编辑/删除/完成/撤销，0.5h 精度工时管理
- **周期切换** — 自定义周范围，历史任务持久化存储，切换周期仅切换视图不丢数据
- **下周计划** — 当周填写下周计划，下周自动弹窗提醒同步，免重复录入
- **周报生成** — 调用大模型（DeepSeek / 通义千问 / OpenAI 兼容接口）一键生成书面化周报，保留技术细节（表名、字段名、方案名）
- **历史查询** — 按周范围、项目、状态、关键词多条件筛选回顾历史任务
